...
ALLOWED_STATUSES = {"pending_review", "active", "frozen", "closed"}
...
def list_accounts(
    ...
    if currency:
        ...
        query["currency"] = currency_upper
    if status:
        if status not in ALLOWED_STATUSES:
            raise HTTPException(status_code=400, detail=f"Invalid status. Use: {ALLOWED_STATUSES}")
        query["status"] = status
